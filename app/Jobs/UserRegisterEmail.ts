...
export default class UserRegisterEmail implements JobContract {
    ...
    public async handle(UserRegisterEmail) {
        ...
        try {
            ...
            const res = await Mail.use("smtp").send(
                message => {
                    message.to("vermavarun932@gmail.com").from("Peter@adonis.com").subject("Welcome To Team!").htmlView(`user_registered`, { user }); // Probably we will fetch email from user
                },
                {
                    transaction: true,
                    openTracking: false
